...
```bash
npm run -w server prisma:migrate       # Create new migration
npm run -w server prisma:migrate:deploy # Deploy migrations
npm run -w server prisma:studio        # Open Prisma Studio
npm run -w server playwright:install   # Install Chromium for screenshots
npm run -w server screenshots:regenerate # Regenerate all approved screenshots
```
...
Engines are initialized by `AppRuntime` (the composition root at `client/src/app/runtime/AppRuntime.ts`), which orchestrates lifecycle, settings, share workflows, and rendering.

### Sandbox Protocol

Textmode visuals execute in an **iframe runner** for isolation. Communication uses a typed message protocol defined in `client/src/sandbox/protocol.ts`:
...
Zustand store at `client/src/platform/state/appStore.ts` is composed from four domain slices:
...
Environment is validated with Zod in `server/src/config/env.ts`. Reads from root `.env` file.
...
3. Update server route handler in `server/src/modules/`
4. Update client service in `client/src/services/SketchApiService.ts`
5. Update admin types in `client/src/features/admin/` if applicable
...
2. If changing the sandbox protocol, update `client/src/sandbox/protocol.ts` **and** the runner's message handler in `client/src/engines/textmode/runner/`
...
2. Create migration: `npm run -w server prisma:migrate`
...
- Use shadcn/ui primitives from `client/src/shared/ui/`
- Follow existing patterns in `client/src/components/` and `client/src/features/`
...
### ESLint:
- Separate configs per workspace: `client/eslint.config.js`, `server/eslint.config.js`
...
1. **Script**: `client/scripts/extractTypes.ts`
2. **Output**: `client/src/engines/textmode/config/generatedTypes.ts`
3. **Consumer**: `client/src/engines/textmode/editor/TextmodeEditor.ts`
...
- **If Monaco types are missing**: Run `npm run extract-types` and verify `generatedTypes.ts` exists in `client/src/engines/textmode/config/`
- **If `/s/:slug` fails in dev**: Ensure server is running on `PORT` and Vite proxy is active in `client/vite.config.ts`
- **If contracts aren't found**: Run `npm run build -w @synth.textmode.art/contracts` — contracts must build before client/server
- **If Prisma client errors**: Run `npm run -w server prisma:generate` after schema changes
- **If screenshot generation fails**: Ensure Playwright is installed (`npm run -w server playwright:install`) and `SCREENSHOT_BASE_URL` points to a running server
